...
TF_LAYERNORM_EPSILON = 1e-12 # BERT layer norm
...
def bert_masking(inputs,
                 ...
                 mask_token,
                 pad_token: int = -1,
                 mask_condition: Optional[jnp.ndarray] = None,
                 mask_rate: float = 0.15,
                 mask_token_proportion: float = 0.8,
                 random_token_proportion: float = 0.1):
  ...
  Args:
    ...
    mask_condition: An optional extra condition on which tokens can be masked.
  ...
  """
  # Clamps mask_rate into [epsilon, 1].
  mask_rate = jnp.clip(mask_rate, 1e-6, 1)
  ...
  # If we have an extra condition on what can be masked,
  # mark those as pad tokens
  if mask_condition is not None:
    is_pad = jnp.logical_and(is_pad, jnp.logical_not(mask_condition))

  lens = jnp.sum(~is_pad, axis=-1)
  # Obtains the ceiling of the lens to make sure we can mask at least one token.
  mask_lens = jax.lax.ceil(lens * mask_rate).astype(jnp.int32)
  # Positions to mask.
  ...
  cut_off = jnp.take_along_axis(
      sorted_should_mask,
      jnp.expand_dims(mask_lens - 1, 1).astype(jnp.int32),
      axis=-1)
  ...
  # MASK tokens.
  tokens_to_mask = rand < mask_token_proportion

  masked_inputs = jnp.where(tokens_to_mask, fullmask, masked_inputs)
  # Random tokens.
...
class Bias(nn.Module):
  ...
  Similar to hk.Bias (cf. google3/third_party/py/haiku/_src/bias.py).
...
class BertMlmLayer(nn.Module):
  ...
  def __call__(self, last_layer: jnp.ndarray,
    ...
    mlm_hidden = nn.LayerNorm(
        epsilon=TF_LAYERNORM_EPSILON, name='mlm_ln')(
            mlm_hidden)
